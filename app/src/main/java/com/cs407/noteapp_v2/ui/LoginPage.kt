...
import com.cs407.noteapp_v2.data.User
        ...
        { isSuccess, taskException, signedUser ->
            if (isSuccess && signedUser != null)
                if (signedUser.displayName != null && !signedUser.displayName!!.isEmpty()) {
                    var user: User?
                    runBlocking {
                        user = noteDB.userDao().getByUID(signedUser.uid)
                        if (user == null) {
                            noteDB.userDao().insert(User(userUID = signedUser.uid))
                            user = noteDB.userDao().getByUID(signedUser.uid)
                        }
                    }
                    loginButtonClick(UserState(user!!.userId, signedUser.displayName!!, signedUser.uid!!))
                } else {
                    name = true
                }
            else
